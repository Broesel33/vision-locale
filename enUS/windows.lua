local L = WowVision:translate("enUS")

--Bars
L["Bars"] = true
L["Action Bar"] = true
L["Bottom Left Bar"] = true
L["Bottom Right Bar"] = true
L["Right Bar"] = true
L["Right Bar 2"] = true
L["Pet Bar"] = true
L["Stance Bar"] = true

L["Auto Casting"] = true
L["Not Auto Casting"] = true

--Bags and containers
L["Bags"] = true
L["Bag Slot"] = true
L["Containers"] = true
L["Bank Bag Slots"] = true

--Character
L["Character"] = true

--Skills
L["Skills"] = true
L["Skill Details"] = true
L["Collapse All"] = true
L["Expand All"] = true
L["Unlearn"] = true

--Reputation
L["Reputation"] = true
L["At War"] = true
L["Watched"] = true

--Pet
L["Pet"] = true
L["No pet"] = true
L["Diet"] = true
L["Stats"] = true
L["Resistances"] = true

--PVP
L["PVP"] = true
L["Honorable Kills"] = true
L["Today"] = true
L["Yesterday"] = true
L["Lifetime"] = true

--Talents
L["Talents"] = true
L["Specialization"] = true
L["Talent Grid"] = true
L["rank"] = true

--Collections
L["Collections"] = true
L["Mounts"] = true
L["Pet Journal"] = true
L["Toy Box"] = true
L["Heirlooms"] = true
L["Appearances"] = true
L["Warband Scenes"] = true

--Gossip Frame
L["Gossip"] = true

--Item Text
L["Item Text"] = true

--Item Upgrade
L["Item Upgrade"] = true

--LFG
L["Looking for Group"] = true
L["Browse Groups"] = true
L["New Player Friendly"] = true
L["Activities"] = true
L["Comment"] = true
L["Roles"] = true
L["Members"] = true

--Loot
L["Loot"] = true
L["No loot."] = true

--Macros
L["Macros"] = true
L["Drag to pick up macro"] = true

--Mail
L["Mail"] = true
L["Inbox"] = true
L["From"] = true
L["Subject"] = true
L["Letter"] = true
L["Send To"] = true
L["Send Mail"] = true
L["Attachments"] = true
L["Postage"] = true
L["Body"] = true
L["Money"] = true
L["Send Money"] = true
L["COD"] = true
L["Drop Item"] = true

--Merchant
L["Merchant"] = true
L["in stock"] = true
L["Repair All"] = true
L["Repair Item"] = true
L["Guild Bank Repair"] = true

--popups
L["Popups"] = true
L["Popup"] = true

--popups/readycheck
L["Ready Check"] = true

--popups/rolepoll
L["Role Poll"] = true
L["Tank"] = true
L["Healer"] = true
L["Damage Dealer"] = true

--Quest
L["Quest Window"] = true
L["Quest"] = true

--Quest Log
L["Quest Log"] = true
L["Quest Choice"] = true
L["Daily"] = true
L["Weekly"] = true
L["Quest List"] = true
L["Quests"] = true

--Range
L["Range"] = true
L["Range Changed"] = true

--Reforging
L["Reforging"] = true

--Spellbook
L["Spellbook"] = true
L["Spells"] = true
L["Previous Page"] = true
L["Next Page"] = true

--Taxi
L["Taxi"] = true

--TradeSkill
L["Trade Skill"] = true
L["Recipes"] = true

--Training
L["Training"] = true
L["Macro Text"] = true
L["Friends"] = true
L["Away"] = true
L["Busy"] = true
L["Note"] = true
L["Friend request"] = true
L["Controls"] = true
L["Column"] = true
L["This tab is not supported yet"] = true
L["Bag Menu"] = true
L["Sort Bags"] = true
L["Quest Item"] = true
L["Junk"] = true
L["Filtered"] = true
L["Locked"] = true
L["Add Slots"] = true
L["Bag Controls"] = true
L["Bag Slots"] = true
L["Color"] = true
L["Item Quality Colors"] = true
L["Raid Frame Preview"] = true
L["Nameplate Preview"] = true
L["Hex Color"] = true
L["Color Picker"] = true
L["Current Color"] = true
L["Original Color"] = true
L["Opacity"] = true
L["Bag Shape"] = true
L["Grid"] = true
L["Equipped"] = true
L["%d items missing"] = true
L["Equipment Sets"] = true
L["Sidebar Tabs"] = true
L["PvP"] = true
L["Currency"] = true
L["Statistics"] = true
L["Next Reward"] = true
L["Edit Set"] = true
L["Delete Set"] = true
